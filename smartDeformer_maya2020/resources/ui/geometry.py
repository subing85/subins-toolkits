...
from smartDeformer_maya2020 import resources
from smartDeformer_maya2020.modules import cluster
from smartDeformer_maya2020.modules import skincluster
from smartDeformer_maya2020.modules import studioMaya
from smartDeformer_maya2020.utils import generic
from smartDeformer_maya2020.utils import read
...
class Geometry(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        for each_data in [self.source_data, self.target_data]:
            ...
            self.verticallayout_geometry.setContentsMargins(
                1, 1, 1, 1
            )
            ...
            self.horizontallayout_inputs.setContentsMargins(
                1, 1, 1, 1
            )
            ...
            self.lineedit.setObjectName(
                "lineedit_%s" % self.titles[index]
            )
            self.lineedit.setReadOnly(True)
            self.lineedit.setToolTip(
                "%s Geometry" % self.titles[index]
            )
            ...
            for x, items in sort_data.items():
                for each in items:
                    ...
                    icon = QtGui.QIcon()
                    icon.addPixmap(
                        QtGui.QPixmap(current_icon_path),
                        QtGui.QIcon.Normal,
                        QtGui.QIcon.Off,
                    )
                    ...
                    ing += 1
            self.deformer_index.setdefault(
                self.titles[index], deformer_inputs
            )
            self.treewidget = QtWidgets.QTreeWidget(self.groupbox)
            self.treewidget.setObjectName(
                "treeWidget_%s" % self.titles[index]
            )
    ...
    def pop_menu(self, treewidget):  # custom Context Menu
        ...
        self.action_lock = QtWidgets.QAction(self)
        self.action_lock.setText("Lock Weights")
        self.pop_menu.addAction(self.action_lock)
        self.action_unlock = QtWidgets.QAction(self)
    ...
    def add_object(self, lineedit, treewidget, tag, combobox):
        lineedit.clear()
        shapes = self.my_maya.getSelectedObjectShapeNode(
            OpenMaya.MFn.kMesh
        )
        if not shapes:
        ...
        self.set_current_deformer(
            lineedit, treewidget, tag, current_index
        )
    ...
    def create_items(self, parent, text, icon_name):
        current_item = QtWidgets.QTreeWidgetItem(parent, icon_name)
        iconPath = ""
        icon = QtGui.QIcon()
        icon.addPixmap(
    ...
    def set_current_deformer(
        self, lineedit, treewidget, tag, current_index
    ):
        ...
        if (
            current_deformer_index[current_index]
            not in self.my_maya.object_types
        ):
            return
        current_type = self.my_maya.object_types[
            current_deformer_index[current_index]
        ]
        deformers = self.my_maya.getDeformerNodes(
            current_mesh, current_type
        )
        ...
        if current_deformer_index[current_index] == "skincluster":
            current_skincluster = self.my_maya.getSkincluster(
                current_mesh
            )
        ...
        if current_deformer_index[current_index] == "wire":
            ...
            for index in range(deformers.length()):
                ...
                joint_data = self.my_maya.getDeformerJoints(
                    curve_objects
                )
                cluster_data = self.my_maya.getClusterHandle(
                    curve_objects
                )
                for k, v in joint_data.items():
                    ...
                    for index in range(v.length()):
                        if (
                            ...
                            in deformer_data[curve_node.name()]
                        ):
                            continue
                        deformer_data.setdefault(
                            curve_node.name(), []
                        ).append(v[index].fullPathName())
                for k, v in cluster_data.items():
                    curve_node = OpenMaya.MFnDependencyNode(k)
                    deformer_data.setdefault(curve_node.name(), [])
                    for index in range(v.length()):
                        cluster_node = OpenMaya.MFnDependencyNode(
                            v[index]
                        )
                        if (
                            cluster_node.name()
        ...
        if current_deformer_index[current_index] == "lattice":
            ...
            for k, v in joint_data.items():
                ...
                for index in range(v.length()):
                    if parent_node.name() in deformer_data:
                        if (
                            v[index].fullPathName()
                            in deformer_data[parent_node.name()]
                        ):
                            continue
                    deformer_data.setdefault(
                        parent_node.name(), []
            ...
            for k, v in cluster_data.items():
                ...
                for index in range(v.length()):
                    ...
                    deformer_data.setdefault(
                        parent_node.name(), []
                    ).append(child_node.name())
        if current_deformer_index[current_index] == "cluster":
            for index in range(deformers.length()):
                mfn_dag_cluster = OpenMaya.MFnDagNode(
                    deformers[index]
                )
                cluster_mobject = mfn_dag_cluster.parent(0)
                cluster_handle = OpenMaya.MFnDependencyNode(
                    cluster_mobject
                )
        ...
        for each_key in keys:
            parent_item = self.create_items(
                treewidget, each_key, "test"
            )
            ...
            for each_value in values:
                child_item = self.create_items(
                    parent_item, each_value, "test"
                )

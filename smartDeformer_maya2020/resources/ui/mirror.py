...
class Mirror(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        for each_mirror in mirrors:
            ...
            self.horizontallayout_mirror = QtWidgets.QHBoxLayout(
                self.groupbox
            )
            ...
            self.horizontallayout_mirror.setContentsMargins(
                20, 20, 20, 20
            )
            self.radiobutton_x = QtWidgets.QRadioButton(self.groupbox)
            self.radiobutton_x.setObjectName(
                "radiobutton_x_%s" % each_mirror
            )
            ...
            self.radiobutton_y.setObjectName(
                "radiobutton_y_%s" % each_mirror
            )
            ...
            self.radiobutton_z.setObjectName(
                "radiobutton_z_%s" % each_mirror
            )
            ...
            self.button_mirror.setObjectName(
                "button_mirror_%s" % each_mirror
            )
            ...
            self.button_flip.setObjectName(
                "button_flip_%s" % each_mirror
            )
            ...
            self.horizontallayout_copy.setContentsMargins(
                20, 20, 20, 20
            )
            ...
            self.button_copy.setObjectName(
                "button_copy_%s" % each_mirror
            )
    ...
    def set_mirror_flip(self, tag, type, radiobuttons):
        ...
        if radiobuttons[2].isChecked():
            axis = [1, 1, -1]
        if tag == "cluster":
            self.cluster.create_mirror_flip(
                selection_array, axis, type
            )
        if tag == "skincluster":
            self.skincluster.create_mirror_flip(
                selection_array, axis, type
            )
    ...
    def copy(self, tag):
        ...
        if selection_array.length() < 2:
            OpenMaya.MGlobal.displayWarning(
                "You need to select minimum two %s!..."
                % tag_result[tag]
            )
            return
        ...
        if tag == "cluster":
            ...
            for index in range(1, selection_array.length()):
                targets_deformer.append(selection_array[index])
            self.cluster.copy_weights(
                selection_array[0], targets_deformer
            )
        if tag == "skincluster":
            if selection_array.length() != 2:
                OpenMaya.MGlobal.displayWarning(
                    "You need to select only two %s!..."
                    % tag_result[tag]
                )

...
from smartDeformer_maya2020.modules import readWrite
...
class Weights(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.verticallayout_weight = QtWidgets.QVBoxLayout(
            self.groupbox
        )
        self.verticallayout_weight.setObjectName(
            "verticallayout_weight"
        )
        ...
        self.horizontallayout_weight.setObjectName(
            "horizontallayout_weight"
        )
        self.horizontallayout_weight.setSpacing(1)
        self.horizontallayout_weight.setContentsMargins(1, 1, 1, 1)
        self.verticallayout_weight.addLayout(
            self.horizontallayout_weight
        )
    ...
    def pop_menu(self, listwidget):
        ...
        for each_actions in [self.action_rename, self.action_remove]:
            ...
            current_icon = os.path.join(
                resources.getIconPath(), "{}.png".format(icon_name)
            )
            icon = QtGui.QIcon()
            icon.addPixmap(
    ...
    def exports(self):
        ...
        if None in drivers:
            ...
            return
        if drivers.keys().count(drivers.keys()[0]) != len(
            drivers.keys()
        ):
            OpenMaya.MGlobal.displayError(
    ...
    def imports(self):
        ...
        for each_item in self.listwidget.selectedItems():
            if str(each_item.text()) not in self.bundles:
                OpenMaya.MGlobal.displayWarning(
                    "\nCorresponding weight not found %s"
                    % each_item.text()
                )
                continue

...
class Cluster(studioMaya.Maya):
    ...
    def read_weight(self, mcluster):
        mfn_weight_filter = OpenMayaAnim.MFnWeightGeometryFilter(
            mcluster
        )
    ...
    def write_weight(self, cluster_mobject, geometry_data):
        for (
            each_geometry,
            each_data,
        ) in geometry_data.items():
            try:
                dag_path = self.getDagPath(each_geometry)
            except:
                OpenMaya.MGlobal.displayWarning(
                    '\nCan not find object called "%s"'
                    % each_geometry
                )
                continue
            dag_path = self.getDagPath(each_geometry)
            ...
            mscript_util.createFloatArrayFromList(
                weights, mfloat_array
            )
            self.setClusterWeights(
                dag_path, cluster_mobject, mfloat_array
            )
    ...
    def create_mirror_flip(self, cluster_dag_paths, axis, tag):
        for index in range(cluster_dag_paths.length()):
            position = self.getClusterPosition(
                cluster_dag_paths[index]
            )
            ...
            symmetry_weights = self.get_flip_weights(
                weight_data, axis
            )
            ...
            if tag == "mirror":
                ...
                target_position = [
                    mirror_position[0] * proxy_position[0],
    ...
    def get_flip_weights(self, geometry_data, axis):
        ...
        for each_geometry, weights_data in geometry_data[
            "geometry"
        ].items():
            ...
            for index in range(len(weights)):
                ...
                symmetry_weights[each_geometry]["weights"][
                    symmetry_vertex_id
                ] = weights[index]
                symmetry_weights[each_geometry]["memberships"][
                    symmetry_vertex_id
                ] = memberships[index]
    ...
    def merge_weights(self, weight_a, weight_b):
        ...
        for each_geometry, geometry_data in weight_a.items():
            ...
            for index in range(len(weights_a)):
                ...
                membership_index_b = weight_b[each_geometry][
                    "memberships"
                ]
                merge_weight = (
                    weights_a[index] + weight_index_b[index]
                )
                if merge_weight > 1:
                    merge_weight = 1
                current_memberships = True
                if (
                    not memberships_a[index]
                    and not membership_index_b[index]
                ):
                    current_memberships = False
                merge_weight_data[each_geometry]["weights"][
                    index
                ] = merge_weight
                merge_weight_data[each_geometry]["memberships"][
                    index
                ] = current_memberships
    ...
    def combine_weights(self, cluster_dag_paths):
        ...
        for index in range(cluster_dag_paths.length()):
            weight_data = self.get_weight(cluster_dag_paths[index])
            for each_geometry, geometry_data in weight_data[
                "geometry"
            ].items():
                weights = geometry_data["weights"]
                memberships = geometry_data["memberships"]
                weights_data.setdefault(each_geometry, []).append(
                    weights
                )
                memberships_data.setdefault(each_geometry, []).append(
                    memberships
                )
        geometry_data = {}
        ...
        position = self.getCenterPosition(
            cluster_dag_paths, "cluster"
        )
        cluster, clusterHandle = self.create(
            "combine_cluster", clear=True
        )
        self.setClusterPosition(clusterHandle, position)
    ...
    def blend_shape(self):
        ...
        loop = min(
            [len(self.target_deformers), len(self.source_deformers)]
        )
        for index in range(loop):
            ...
            if not self.target_deformers[index]:
                ...
                name = (
                    "%s_blend_shape_cluster"
                    % self.source_deformers[index].replace(".", "_")
                )
    ...
    def to_cluster(self):
        ...
        for index in range(loop):
            _targt_deformer = self.target_deformers[index]
            if not self.target_deformers[index]:
                name = "%s_cluster_cluster" % self.source_deformers[
                    index
                ].replace(".", "_")
                cluster, clusterhandle = self.create(name, clear=True)
            ...
            weight_data = self.get_weight(
                self.source_deformers[index]
            )
            weight_object = weight_data["geometry"].keys()[0]
            weights = weight_data["geometry"][weight_object][
                "weights"
            ]
    ...
    def skin_cluster(self):
        ...
        for index in range(loop):
            ...
            if not self.target_deformers[index]:
                ...
                self.setClusterPosition(clusterhandle, [x, y, z])
                _targt_deformer = clusterhandle
            joint_dag_path = self.getDagPath(
                self.source_deformers[index]
            )
            cluster_mobject = self.getDependences(
                _targt_deformer, OpenMaya.MFn.kClusterFilter
    ...
    def to_specific_deformer(self, tag):
        ...
        for index in range(loop):
            source_dag_path = self.getDagPath(
                self.source_deformers[index]
            )
        ...
        for index in range(source_deformer_dag_path.length()):
            ...
            if source_dag_path.hasFn(
                ...
                for k, v in curve_weights.items():
                    ...
                    cluster, clusterhandel = self.create(
                        name, clear=True
                    )
                    self.setClusterPosition(
                        clusterhandel, v["position"]
                    )
                    self.setClusterWeights(
                        self.target_geometrys[0],
                        cluster,
            ...
            ) or source_dag_path.hasFn(OpenMaya.MFn.kJoint):
                _targt_deformer = self.target_deformers[index]
                if source_dag_path.hasFn(OpenMaya.MFn.kJoint):
                    ...
                    children = self.getChildren(
                        source_dag_path,
                        mfn_shape=OpenMaya.MFn.kCluster,
                    )

...
class Shader(studioMaya.Maya):
        ...
        for index, shader_data in data.items():
            try:
                self.create_shader_net(
                    shader_data,
                    assign=assign,
                    assign_components=maya_objects,
                )
    ...
    def save(self, file_path, name, image, user_comment=None):
        mobject_arrary = self.getObjectShadingEngine(
            self.geometry_dag_path
        )
        net_data = {}
        ...
        description = (
            "This data contain information about maya shader networks"
        )
        type = "shader_net"
        valid = True
        data = net_data
    ...
    def get_shader_net(self, shading_engine):
        ...
        for each_node in nodes:
            ...
            for each_attribute in attributes:
                ...
                attribute_data.setdefault(
                    each_node, current_attribute_data
                )
            # get connections
            current_connection_data = {}
            connections = py_node.listConnections(
            ...
            for each_connection in connections:
                ...
                current_connection_data.setdefault(
                    source_attribute[0].longName().encode(), []
        ...
        set_name, component_data = self.getAssignComponents(
            shading_engine
        )
    ...
    def create_shader_net(
        self, shader_data, assign=True, assign_components=None
    ):
        ...
        for (
            each_node,
            current_pynode,
        ) in py_nodes.items():  # set attributes values
            if not attribute_data:
                continue
            if each_node not in attribute_data:
                continue
            for each_attribute, values in attribute_data[
                each_node
            ].items():
                if not core.objExists(
                    "%s.%s" % (current_pynode.name(), each_attribute)
                ):
                    continue
                current_attribute = current_pynode.attr(
                    each_attribute
                )
                try:
        ...
        for (
            each_node,
            current_pynode,
        ) in py_nodes.items():  # set connections
            if not connection_data:
                continue
            if each_node not in connection_data:
                continue
            for parent_attribute, children in connection_data[
                each_node
            ].items():
                ...
                for each_child in children:
                    ...
                    if not current_child:
                        continue
                    try:
                        current_parent_attribute.connect(
                            current_child, f=True
                        )
                    except Exception as error:
                        print 'shader libaray warning "connect attri" {}\t{}\n\t{}'.format(
                            current_parent_attribute,
                            each_child,
                            error,
                        )

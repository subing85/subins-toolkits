...
class MayaWindow(QtGui.QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.horizontallayout_bar = QtGui.QHBoxLayout(
            self.groupbox_bar
        )
        self.horizontallayout_bar.setObjectName(
        ...
        self.horizontallayout_input.setObjectName(
            "horizontallayout_input"
        )
        ...
        self.textedit_output.setLineWrapMode(
            QtGui.QTextEdit.WidgetWidth
        )
        ...
        self.horizontallayout_credit.setObjectName(
            "horizontallayout_credit"
        )
        ...
        self.action_import_sample.setObjectName(
            "action_import_sample"
        )
        ...
        self.treewidget_maya.setContextMenuPolicy(
            QtCore.Qt.CustomContextMenu
        )
        self.treewidget_maya.customContextMenuRequested.connect(
            partial(
                self.on_context_menu, self.treewidget_maya, "maya"
            )
        )
        ...
        self.treewidget_code.setContextMenuPolicy(
            QtCore.Qt.CustomContextMenu
        )
        self.treewidget_code.customContextMenuRequested.connect(
            partial(
                self.on_context_menu, self.treewidget_code, "code"
            )
        )
        ...
        self.action_remove.triggered.connect(
            partial(self.remove, "selected")
        )
        self.action_clear.triggered.connect(
            partial(self.remove, "all")
        )
    ...
    def modify_widgets(self):
        ...
        icon = QtGui.QIcon()
        icon.addPixmap(
            QtGui.QPixmap(os.path.join(icon_path, "title.png"))
        )
        ...
        for qaction in qactions:
            ...
            icon.addPixmap(
                QtGui.QPixmap(
                    os.path.join(icon_path, "%s.png" % label)
                ),
                QtGui.QIcon.Normal,
                QtGui.QIcon.Off,
            )
    ...
    def save_to(self):
        ...
        data = {
            ...
            "child": {"maya": maya_data, "code": code_data},
        }
    ...
    def os_process(self, mayapy, maya, code, save):
        ...
        if self.operating_system == "Windows":
            if "Program Files" in mayapy:
                mayapy = mayapy.replace(
                    "Program Files", '"Program Files"'
                )
            command = (
                mayapy + " -c " + '"' + "; ".join(commands) + '"'
            )
        os.system(command)

...
class Window(QtGui.QWidget):
    ...
    def setup_ui(self):
        ...
        self.setWindowTitle(
            "%s %s Preference" % (self.label, self.version)
        )
        ...
        self.gridLayout_versions.addWidget(
            self.label_directory, 2, 0, 1, 1
        )
        self.lineedit_directory = QtGui.QLineEdit(
            self.groupbox_versions
        )
        self.lineedit_directory.setObjectName("lineedit_directory")
        self.gridLayout_versions.addWidget(
            self.lineedit_directory, 2, 1, 1, 2
        )
        self.button_directory = QtGui.QPushButton(
            self.groupbox_versions
        )
        ...
        self.gridLayout_settings = QtGui.QGridLayout(
            self.groupbox_mode
        )
        ...
        self.gridLayout_settings.addWidget(
            self.checkbox_query, 0, 0, 1, 1
        )
        ...
        self.gridLayout_settings.addWidget(
            self.checkbox_overwrite, 1, 0, 1, 1
        )
        ...
        self.checkbox_config.clicked.connect(
            partial(
                self.set_versions,
                self.checkbox_config,
                self.combobox_maya,
        ...
        self.button_directory.clicked.connect(
            partial(self.set_directory, self.lineedit_directory)
        )
    ...
    def get_preference_data(self, input="json"):
        ...
        if input == "xml":
            ...
            input_data = {
                ...
                "mode": settings_data,
                "current_version": {"index": index, "name": name},
            }
        else:
    ...
    def apply(self):
        ...
        if not os.path.isfile(maya_path):
            ...
            return
        if (
            not self.checkbox_query.isChecked()
            and not self.checkbox_edit.isChecked()
        ):
            QtGui.QMessageBox.warning(
                self,
                "Warning",
            ...
            return
        if (
            not self.checkbox_overwrite.isChecked()
            and not self.checkbox_version.isChecked()
        ):
            QtGui.QMessageBox.warning(
                self,
                "Warning",
        ...
        QtGui.QMessageBox.information(
            self, "Information", "Success!...", QtGui.QMessageBox.Ok
        )
        self.labels

...
from assetLibrary.modules import studioFolder
from assetLibrary.modules import studioAsset
from assetLibrary.modules import studioPrint
from assetLibrary.resources.ui import preferences
from assetLibrary.resources.ui import catalogue
from assetLibrary.resources.ui import asset
...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None, standalone=False):
        ...
        (
            ...
            self.tool_kit_name,
            self.version,
        ) = platforms.get_tool_kit()
        self.tool_kit_titile = "{} {}".format(
    ...
    def setup_ui(self):
        ...
        title_icon = os.path.join(
            resources.getIconPath(), "title.png"
        )
        ...
        if not self.standalone:
            ...
            self.pushbutton_filepath.customContextMenuRequested.connect(
                partial(
                    self.on_context_path, self.pushbutton_filepath
                )
            )
            ...
            self.action_scene.triggered.connect(
                partial(
                    self.set_current_scene, self.lineedit_filepath
                )
            )
        ...
        self.pushbutton_imagepath.clicked.connect(
            partial(
                self.set_source_image_path, self.lineedit_imagepath
            )
        )
        ...
        self.menu_default = QtGui.QMenu(self.menu_file)
        ...
        self.action_abouttoolkits.setObjectName(
            "action_abouttoolkits"
        )
        ...
        self.preference.button_apply.clicked.connect(
            self.set_preference
        )
        self.preference.button_cancel.clicked.connect(
            self.cancel_preference
        )
    ...
    def set_icons(self):
        ...
        qmenus = self.findChildren(QtGui.QMenu)
        for each_action in actions + qmenus:
            ...
            icon_path = os.path.join(
                resources.getIconPath(), current_icon
            )
            icon = QtGui.QIcon()
            icon.addPixmap(
    ...
    def default_menu(self, parent):
        ...
        for index in range(len(order_data)):
            if not data[order_data[index]]["enable"]:
                continue
            action_default = QtGui.QAction(self)
            action_default.setObjectName(
                "action_{}".format(order_data[index])
            )
            action_default.setText(
                data[order_data[index]]["long_name"]
            )
            action_default.setToolTip(
                data[order_data[index]]["tooltip"]
            )
    ...
    def set_current_scene(self, linedeit):
        ...
        self.source_file, self.source_file_path = True, str(
            current_scene
        )
    ...
    def load_current_folder(self, treewidget, *args):
        ...
        self.load_publish_to_layout(
            self.listwidget, self.dependent_list
        )
        self.tool_mode = "publish"
        ...
        for each_item in items:
            ...
            for each_publish in publish_datas:
                if not os.path.isfile(
                    os.path.join(curren_path, each_publish)
                ):
                    continue
                if not each_publish.endswith(
                    ".%s" % self.publish_format
                ):
                    continue
    ...
    def set_source_file_path(self, widget):
        ...
        ) = self.asset.set_source_path(widget, "file")
    ...
    def publish(self):
        ...
        if not os.path.isdir(current_path):
            QtGui.QMessageBox.warning(
                self,
                "Warning",
                "Not found such Publish directory!...%s"
                % current_path,
                QtGui.QMessageBox.Ok,
            )
            self.studio_print.display_warning(
        ...
        self.source_file_path = os.path.abspath(
            str(self.lineedit_filepath.text())
        ).replace("\\", "/")
        studio_asset = studioAsset.Asset(
        ...
        if False in result:
            message = "Publish Failed!...\n%s\n%s" % (
                result[False],
                "[more details and debugging subing85@gmail.com]",
            )
    ...
    def builds(self, tag, listwidge, *args):
        ...
        self.lineedit_label.setText(
            os.path.basename(os.path.splitext(publish_paths[-1])[0])
        )
        self.asset.image_to_button(
        ...
        if tag == "build":
            ...
            if self.standalone:
                ...
                result = studio_asset.create(
                    "standalone",
                    self.create_type,
                    maya_type=self.maya_type,
                    maya_path=self.maya_path,
                ...
                result = studio_asset.create("maya", self.create_type)
                QtGui.QApplication.restoreOverrideCursor()

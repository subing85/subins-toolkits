    ...
    for index, contents in uv_data.items():
        ...
        for content in uv_contents:
            try:
                ...
                imported_result.setdefault(True, []).append(
                    content["shape_node"]
                )
            except Exception as error:
                imported_result.setdefault(False, []).append(
                    content["shape_node"]
                )
                set_data = None
            if not set_data:
...
def find_from_scene(data, mode=False, polygons=None):
    ...
    for index, contents in data.items():
        for polygon in polygons:
            if isinstance(polygon, str) or isinstance(
                polygon, unicode
            ):
            ...
            (
                num_polygons,
                polygon_vertices,
            ) = studio_maya.getFacesVertices(mfn_mesh)
            ...
            scene_contents["short_name"] = polygon.fullPath().split(
                "|"
            )[-1]
            scene_polygons.setdefault(index, []).append(
                scene_contents
            )
...
def find_from_data(data, polygons=None):
    ...
    for index, contents in data.items():
        ...
        if not current_node:
            continue
        node_instances = [
            each.fullPath() for each in core.ls(current_node)
        ]

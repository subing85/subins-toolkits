...
from smartDeformer_maya2023.utils import config
...
def has_valid():
    ...
    if tool_app not in application:
        result = {False: 'Only support "%s %s"' % (tool_app, tool_ver)}
        return result
...
def get_qwidget():
    ...
    main_window = shiboken2.wrapInstance(swig, QtWidgets.QWidget)
...
def get_main_window():
    ...
    maya_windows = [
        each_win for each_win in cmds.lsUI(wnd=True) if cmds.window(each_win, q=True, mw=True)
    ]

...
class Mirror(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        for each_mirror in mirrors:
            ...
            self.horizontallayout_mirror.setObjectName("horizontallayout_mirror_%s" % each_mirror)
            ...
            self.horizontallayout_copy.setObjectName("horizontallayout_copy_%s" % each_mirror)
    ...
    def set_mirror_flip(self, tag, type, radiobuttons):
        ...
        if not selection_array.length():
            OpenMaya.MGlobal.displayWarning("Your select not contain %s!..." % tag)
            return
        axis = [1, 1, 1]
    ...
    def copy(self, tag):
        ...
        if not selection_array.length():
            OpenMaya.MGlobal.displayWarning("Your select not contain %s!..." % tag_result[tag])
            return

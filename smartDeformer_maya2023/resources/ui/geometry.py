...
from smartDeformer_maya2023 import resources
from smartDeformer_maya2023.modules import cluster
from smartDeformer_maya2023.modules import skincluster
from smartDeformer_maya2023.modules import studioMaya
...
class Geometry(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        for each_data in [self.source_data, self.target_data]:
            ...
            self.verticallayout_geometry.setObjectName("verticallayout_%s" % self.titles[index])
            ...
            deformer_inputs = {}
            ing = 1
            for x in each_data["data"]:
                current_icon_path = os.path.join(self.icon_path, "%s.png" % x["code"])
                icon = QtGui.QIcon()
                icon.addPixmap(
                ...
                self.combobox.addItem(icon, x["label"])
                deformer_inputs.setdefault(ing, x["code"])
                ing += 1
            ...
            self.treewidget.itemDoubleClicked.connect(partial(self.select_items, self.treewidget))
        ...
        self.button.clicked.connect(partial(self.convert, self.lineedit, self.treewidget))

    def pop_menu(self, treewidget):  # custom Context Menu
        treewidget.setContextMenuPolicy(QtCore.Qt.CustomContextMenu)
        treewidget.customContextMenuRequested.connect(partial(self.context_menu, treewidget))
    ...
    def set_current_deformer(self, lineedit, treewidget, tag, current_index):
        ...
        if current_deformer_index[current_index] == "wire":
            ...
            for index in range(deformers.length()):
                ...
                for curve_node, v in joint_data.items():
                ...
                for curve_node, v in cluster_data.items():
                    # curve_node = OpenMaya.MFnDependencyNode(k)
                    deformer_data.setdefault(curve_node.name(), [])
                    for index in range(v.length()):
        ...
        if current_deformer_index[current_index] == "lattice":
            ...
            for parent_node, v in cluster_data.items():
                for index in range(v.length()):
    ...
    def convert(self, lineedit, treewidget):
        ...
        if self.target_deformer[0] == 1:  # to cluster
            ...
            if self.source_deformer[0] == 1:  # to softSelection
                ...
                OpenMaya.MGlobal.displayInfo("Soft selection to Cluster Success!..")
            ...
            if self.source_deformer[0] == 2:  # to blendShape
                try:
                    ...
                    OpenMaya.MGlobal.displayInfo("Blendshape to Cluster Success!..")
                except Exception as error:
                    raise Exception(error)
            ...
            if self.source_deformer[0] == 4:  # to lattice
                try:
                    my_cluster.lattice()
                    OpenMaya.MGlobal.displayInfo("Lattice to Cluster Success!..")
                except Exception as error:
                    raise Exception(error)
            ...
            if self.source_deformer[0] == 5:  # to cluster
                try:
                    ...
                    OpenMaya.MGlobal.displayInfo("Cluster to Cluster Success!..")
                except Exception as error:
                    raise Exception(error)
            ...
            if self.source_deformer[0] == 6:  # to skincluster
                try:
                    ...
                    OpenMaya.MGlobal.displayInfo("Cluster to Skincluster Success!..")
                except Exception as error:
                    raise Exception(error)

        if self.target_deformer[0] == 2:  # to skincluster
            m_skinclusters = self.my_maya.getSkincluster(self.target_geometry)
            ...
            if self.source_deformer[0] == 1:  # to softSelection
                try:
                    ...
                    OpenMaya.MGlobal.displayInfo("Soft selection to Skincluster Success!..")
                except Exception as error:
                    raise Exception(error)
            else:
                if not self.source_deformers:
            ...
            if self.source_deformer[0] == 2:  # to blendShape
                try:
                    ...
                    OpenMaya.MGlobal.displayInfo("Blendshape to Skincluster Success!..")
                except Exception as error:
                    raise Exception(error)
            ...
            if self.source_deformer[0] == 3:  # to wire
                try:
                    my_skincluster.wire()
                    OpenMaya.MGlobal.displayInfo("Wire to Skincluster Success!..")
                except Exception as error:
                    raise Exception(error)
            ...
            if self.source_deformer[0] == 4:  # to lattice
                try:
                    ...
                    OpenMaya.MGlobal.displayInfo("Lattice to Skincluster Success!..")
                except Exception as error:
                    raise Exception(error)
            ...
            if self.source_deformer[0] == 6:  # to skincluster
                try:
                    ...
                    OpenMaya.MGlobal.displayInfo("Skincluster to Skincluster Success!..")
                except Exception as error:
                    raise Exception(error)

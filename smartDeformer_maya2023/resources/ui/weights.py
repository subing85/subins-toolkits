...
class Weights(QtWidgets.QWidget):
    ...
    def pop_menu(self, listwidget):
        ...
        listwidget.customContextMenuRequested.connect(partial(self.context_menu, listwidget))
        ...
        for each_actions in [self.action_rename, self.action_remove]:
            ...
            current_icon = os.path.join(resources.getIconPath(), "{}.png".format(icon_name))
            icon = QtGui.QIcon()
            icon.addPixmap(
    ...
    def load_weigts(self):
        ...
        for each_bundle, bundle_data in self.bundles.items():
            ...
            item.setIcon(icon)
            item.setToolTip("\n".join(list(bundle_data["data"].keys())))
    ...
    def exports(self):
        ...
        if None in drivers:
            OpenMaya.MGlobal.displayError("#Unwanted nodes are found in your selection")
            return
        if list(drivers.keys()).count(list(drivers.keys())[0]) != len(list(drivers.keys())):
        ...
        weights = {}
        if list(drivers.keys())[0] == "cluster":
            weights = self.cluster.get_weights(drivers["cluster"])
        if list(drivers.keys())[0] == "skincluster":
        ...
        print(rw.file_path)
        self.load_weigts()
        OpenMaya.MGlobal.displayInfo("Export {} weights Done!...".format(list(drivers.keys())[0]))
    ...
    def imports(self):
        ...
        OpenMaya.MGlobal.displayInfo("Import {} Done!...".format(current_weights["tag"]))
    ...
    def rename_item(self):
        ...
        new_weight_path = os.path.join(
            os.path.dirname(weight_path),
            "%s.%s" % (file_name, rw.extention),
        )
        try:
    ...
    def remove_items(self):
        ...
        replay = QtWidgets.QMessageBox.question(
            self,
            "Question",
            "Are you sure, you want to remove the weights?",

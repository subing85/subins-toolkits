...
class Maya(object):
    ...
    def hasMembership(
        self, deformer_mobject, geometry_dag_path, geometry_component
    ):
        mfn_geometry_filter = OpenMayaAnim.MFnGeometryFilter(
            deformer_mobject
        )
        ...
        if not selectionList.hasItem(
            geometry_dag_path, geometry_component
        ):
            return False
        return True
    ...
    def getDependences(
        self, mobject, node_type, upstream=False, downstream=False
    ):
    ...
    def getConnectedNode(self, node_type, mess=False):
        ...
        while nodes:
            ...
            for x in range(attribute_count):
                ...
                obj_dependency_node = OpenMaya.MFnDependencyNode(
                    mobject
                )
    ...
    def getBlenshapeAttributes(self, mobjects):
        ...
        for index in range(mobjects.length()):
            ...
            mselection_list.add(
                "%s.weight" % mfndependency_node.name()
            )
    ...
    def getDeformerJoints(self, mobjects):
        ...
        for index in range(mobjects.length()):
            joint_data.setdefault(
                mobjects[index], OpenMaya.MDagPathArray()
            )
    ...
    def getWeightsFromSelection(self):
        ...
        mit_selection_list = OpenMaya.MItSelectionList(
            mselection_list
        )
    ...
    def getWeightsFromEnvelope(
        self, origin_dag_path, envelope_dag_path, attribute=None
    ):
    ...
    def get_symmetry_vertex(
        self, geometry_dag_path, vertex_id, axis=[-1, 1, 1]
    ):
        mfn_mesh = OpenMaya.MFnMesh(geometry_dag_path)
        position_m_point = OpenMaya.MPoint()
        mfn_mesh.getPoint(
            vertex_id, position_m_point, OpenMaya.MSpace.kWorld
        )
        ...
        for each_vertex in face_vertex_array:
            ...
            mfn_mesh.getPoint(
                each_vertex,
                vertex_symmetry_position,
                OpenMaya.MSpace.kWorld,
            )
            vertex_symmetry_vector = OpenMaya.MVector(
                vertex_symmetry_position
            )
    ...
    def setClusterWeights(self, dag_path, cluster_mobject, weights):
        ...
        mfn_geo_filter = OpenMayaAnim.MFnGeometryFilter(
            cluster_mobject
        )
        ...
        weight_geometry_filter.setWeight(
            dag_path, components, weights
        )

    def setMembership(
        self, geomotry_dag_path, deformer_mobject, memberships
    ):
        mfn_geo_filter = OpenMayaAnim.MFnGeometryFilter(
            deformer_mobject
        )
    ...
    def setSkinclusterWeights(
        self, geometry_dag_path, joint_dag_path, weights
    ):
        if not isinstance(geometry_dag_path, OpenMaya.MDagPath):
            geometry_dag_path = self.getDagPath(geometry_dag_path)
        ...
        mfn_skincluster = OpenMayaAnim.MFnSkinCluster(
            skincluster_mobject
        )
        # example joint_index =
    ...
    def getClusterPosition(self, dag_path):
        ...
        m_point = m_transformation_matrix.rotatePivot(
            OpenMaya.MSpace.kWorld
        )
        ...
        for each_geometry in geometrys:
            ...
            if not m_skinclusters.length():
                OpenMaya.MGlobal.displayWarning(
                    '\nCan not find skincluster "%s"'
                    % each_geometry.encode()
                )
                continue
            skincluster = self.getName(m_skinclusters[0])
            OpenMaya.MGlobal.executeCommand(
                "skinCluster -e  -dr 4 -lw true -wt 0 -ai {} {}".format(
                    joint, skincluster
                )
            )
    ...
    def findxIndexFromSkincluster(
        self, mfn_skincluster, joint_dag_path
    ):
    ...
    def getChildren(self, object_dag_path, mfn_shape=None):
        if not isinstance(object_dag_path, OpenMaya.MDagPath):
            object_dag_path = self.getDagPath(object_dag_path)
        if mfn_shape:
            shape_dag_path = self.getShapeNode(
                object_dag_path, mfn_shape
            )
        mfn_dag_node = OpenMaya.MFnDagNode(object_dag_path)
        ...
        for index in range(mfn_dag_node.childCount()):
            ...
            if mfn_shape:
                if not shape_dag_path:
                    continue
                if (
                    shape_dag_path.isValid()
                    and child_dag_Path.isValid()
                ):
                    if (
                        shape_dag_path.apiType()
                        == child_dag_Path.apiType()
                    ):
                        continue
    ...
    def getSkincluster(self, geometry):
        ...
        mcommand_result = OpenMaya.MCommandResult()
        OpenMaya.MGlobal.executeCommand(
            'findRelatedSkinCluster("%s");' % geometry,
            mcommand_result,
            True,
            True,
        )

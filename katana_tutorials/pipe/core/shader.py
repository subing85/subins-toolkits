...
def get_assigned_materials(producer):
    ...
    for location in locations:
        ...
        if not material:
            continue
        material_producer = root_producer.getProducerByPath(
            material[0]
        )
        ...
        if material_producer.getType() != "material":
            continue
        assigned_materials.setdefault(material[0], []).append(
            location
        )
...
def get_scene_materials(producer):
    ...
    """
    material_producers = scenegraph.list_specific_producer(
        producer, "material"
    )
    material_assigned_objects = get_material_assigned_objects(
        producer
    )
    scene_materials = {}
    for material_producer in material_producers:
        ...
        if current_material in material_assigned_objects:
            assigned_objects = material_assigned_objects[
                current_material
            ]
...
def get_object_assigned_material(producer):
    ...
    for location in locations:
        ...
        if material_producer.getType() != "material":
            continue
        assigned_objects.setdefault(
            location.getFullName(), material[0]
        )
    return assigned_objects
...
def get_texture_maps(material_producer):
    ...
    """
    texture_maps = {}
    dependency_nodes = get_material_dependency_nodes(
        material_producer
    )
    if not dependency_nodes:
        return texture_maps
    for dependency_node in dependency_nodes:
        filename = get_node_filename(
            material_producer, dependency_node
        )
        if not filename:
            continue
        sourcename = get_node_sourcename(
            material_producer, dependency_node
        )
        if not sourcename:
...
def get_material_dependency_nodes(material_producer):
    ...
    """
    global_attribute = material_producer.getGlobalAttribute(
        "material.nodes"
    )
...
def get_assigned_shader_networks(producer):
    ...
    for assigned_material in assigned_materials:
        ...
        assigned_shader_networks.setdefault(
            assigned_material, shader_networks
        )
        assigned_locations = get_assigned_locations(
            assigned_materials[assigned_material]
        )
        materail_assigments = {
            "assigned_locations": assigned_locations
        }
        assigned_shader_networks[assigned_material].update(
            materail_assigments
        )
...
def get_scene_shader_networks(producer, polymesh=False):
    ...
    for scene_material in scene_materials:
        ...
        shader_networks = get_shader_networks(material_producer)
        scene_shader_networks.setdefault(
            scene_material, shader_networks
        )
        assigned_locations = get_assigned_locations(
            scene_materials[scene_material], polymesh=polymesh
        )
        materail_assigments = {
            "assigned_locations": assigned_locations
        }
        scene_shader_networks[scene_material].update(
            materail_assigments
        )
...
def get_assigned_locations(object_producers, polymesh=False):
    ...
    for object_producer in object_producers:
        ...
        assigned_locations.setdefault(
            object_producer.getFullName(), typed
        )
...
def get_shader_networks(material_producer):
    ...
    shader_networks = {"nodes": {}, "terminals": {}}
    for dependency_node in dependency_nodes:
        shader_parameters = get_node_parameters(
            material_producer, dependency_node
        )
        shader_networks["nodes"].setdefault(
            dependency_node, shader_parameters
        )
...
def get_material_parameters(material_producer):
    ...
    material_attributes = scenegraph.get_attributes(
        material_producer, input_attributes=attributes
    )
    material_values = scenegraph.get_attribute_typed_values(
        material_producer, material_attributes
    )
    material_parameters = {}
...
def get_node_parameters(material_producer, node):
    ...
    primary_values = scenegraph.get_attribute_typed_values(
        material_producer, attributes[0:3]
    )
    ...
    prameter_attributes = scenegraph.get_attributes(
        material_producer, input_attributes=attributes[3:4]
    )
    parameter_values = scenegraph.get_attribute_typed_values(
        material_producer, prameter_attributes
    )
    for k, v in parameter_values.items():
        paramter = k.rsplit(
            "material.nodes.%s.parameters." % node, 1
        )[1]
        shader_parameters["parameters"].setdefault(paramter, v)
    # get the node connection values
    connection_attributes = scenegraph.get_attributes(
        material_producer, input_attributes=attributes[4:5]
    )
    connection_values = scenegraph.get_attribute_typed_values(
        material_producer, connection_attributes
    )
    for k, v in connection_values.items():
        paramter = k.rsplit(
            "material.nodes.%s.connections." % node, 1
        )[1]
...
def export_shader_networks(producer, export_path):
    ...
    shader_networks = get_scene_shader_networks(
        producer, polymesh=False
    )

...
PATTERN = {"major": 0, "minor": 1, "patch": 2}
...
def get_asset_dependency_versions(
    show_path, category, name, origin, dependency
):
    """
    ...
    """
    origin_path = os.path.join(show_path, "asset", category, name)
    ...
    for origin_version in origin_versions:
        ...
        if not os.path.isfile(manifest):
            dependencies.setdefault("invalid", []).append(
                origin_version
            )
            continue
        with (open(manifest, "r")) as file:
            ...
            dependencies.setdefault(current_dependency, []).append(
                origin_version
            )

...
def xml_fie_to_xml_element(xml_scene):
    ...
    if not os.path.isfile(xml_scene):
        ...
        return
    xml_element, upgraded = NodegraphAPI.LoadElementsFromFile(
        xml_scene, versionUp=True
    )
    return xml_element
...
def xml_string_to_xml_element(xml_string):
    ...
    """
    xml_element, upgraded = NodegraphAPI.LoadElementsFromString(
        xml_string, versionUp=True
    )
    return xml_element
...
def xml_element_to_katana_scene(
    katana_scene, xml_element, force=False
):
    """
...
def xml_file_to_katana_scene(katana_scene, xml_scene, force=False):
    ...
    if not os.path.isdir(os.path.dirname(katana_scene)):
        os.makedirs(os.path.dirname(katana_scene))
    xml_element = xml_fie_to_xml_element(xml_scene)
    result = xml_element_to_katana_scene(
        katana_scene, xml_element, force=force
    )
    return result
...
def make_editable_live_group(klive_group):
    ...
    """
    if isinstance(klive_group, str) or isinstance(
        klive_group, unicode
    ):
...
def look_file_bake(
    klook_bake_node, look_file_path, bake_type=0, force=False
):
    if isinstance(klook_bake_node, str) or isinstance(
        klook_bake_node, unicode
    ):
...
def create_global_variable(label, values, force=False):
    ...
    children = child_group.createChildStringArray(
        "options", len(values), 1
    )

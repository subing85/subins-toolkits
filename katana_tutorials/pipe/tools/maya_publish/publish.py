...
def asset_publish(name, category, type, version, model, lookdev):
    ...
    command = (
        "-frameRange 1001 1002 %s -uvWrite -attrPrefix xgen -worldSpace -root %s -stripNamespaces -file %s "
...
def scene_publish(sequence, shot, scene, version, puppets=None):
    ...
    for model_panel in model_panels:
        core.modelEditor(
            model_panel, edit=True, displayAppearance="boundingBox"
        )
    ...
    for puppet in puppets:
        ...
        for puppet_attribute in puppet_attributes:
            ...
            puppet_contents[puppet_name].setdefault(
                puppet_attribute, value
            )
        category = py_node.getAttr("category")
        name = py_node.getAttr("name")
        model = py_node.getAttr("model")
        model_depnendency = get_lookdev_model_depnendency(
            category, name
        )
        latest_lookdev = "None"
        ...
        command = (
            "-frameRange %s %s %s -uvWrite -attrPrefix xgen -worldSpace -root %s -stripNamespaces -file %s "

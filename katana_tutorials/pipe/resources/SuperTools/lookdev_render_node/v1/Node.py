...
class LookdevRenderNode(NodegraphAPI.SuperTool):
    ...
    def add_parameters(self):
        ...
        pipe_parameter_group = parameter.createChildGroup(
            self.pipe_parameter
        )
        for attribute in attributes:
            child_parameter = pipe_parameter_group.createChildString(
                attribute, ""
            )
        ...
        pipe_parameter_group.getChild("asset_node").setValue(
            "None", 1.0
        )
        pipe_parameter_group.getChild("category").setValue(
            inputs.CATEGORIES[0], 1.0
        )
        pipe_parameter_group.getChild("render_mode").setValue(
            inputs.RENDER_MODES[0], 1.0
        )
        pipe_parameter_group.getChild("current_frame").setValue(
            "Yes", 1.0
        )
        pipe_parameter_group.getChild("color_space").setValue(
            inputs.COLOR_SPACE[0], 1.0
        )
        pipe_parameter_group.getChild("file_extension").setValue(
            inputs.FILE_EXTENSION[0], 1.0
        )
    ...
    def create_output_define(self):
        ...
        output_define = scene.xml_file_to_nodes(
            xml_scene, parent=self
        )
    ...
    def add_node_dependency(self):
        ...
        self.getSendPort("input").connect(

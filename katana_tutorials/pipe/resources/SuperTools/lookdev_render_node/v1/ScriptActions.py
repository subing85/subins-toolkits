...
def load_asset_data(knode):
    ...
    category = kasset_node.getParameter("model_category").getValue(
        1.0
    )
    ...
    for input_attribute in input_attributes:
        attributes.append(
            "geometry.arbitrary.%s.value" % input_attribute
        )
    attribute_values = scenegraph.get_attribute_values(
        producer, attributes
    )
    input_values = {}
    ...
    pipe_parameter.getChild("name").setValue(
        input_values["name"], 1.0
    )
    pipe_parameter.getChild("category").setValue(
        input_values["category"], 1.0
    )
    pipe_parameter.getChild("type").setValue("lookdev", 1.0)
    pipe_parameter.getChild("model").setValue(
        input_values["version"], 1.0
    )
...
def get_publish_path(knode):
    ...
    publish_path = os.path.join(show_path, "asset", category, name)
...
def start_render(knode):
    ...
    render.start_render(
        current_render_node, render_mode, [start_frame, end_frame]
    )
...
def publish_lookdev_scene(knode):
    ...
    scenegraph_location = os.path.join(
        studio_nodes.SCENE_GRAPH_LOCATION,
        input_data["category"],
        input_data["name"],
    )
...
def texture_remapping(knode, location, directory):
    ...
    if not producer:
        message = (
            "not able to find scenegraph location\n%s" % location
        )
        QtGui.QMessageBox.warning(
            None, "Warning", message, QtGui.QMessageBox.Ok
        )
    ...
    for material in texture_maps:
        for texture_map, nodes in texture_maps[material].items():
            source_image_path = os.path.join(
                source_image_dirname, os.path.basename(texture_map)
            )
            ...
            for node in nodes:
                ...
                if not ktexture_node:
                    ...
                    continue
                parameter = ktexture_node.getParameter(
...
def get_input_data(knode):
    ...
    for attribute in attributes:
        parameter_value = pipe_parameter.getChild(attribute).getValue(
            1.0
        )

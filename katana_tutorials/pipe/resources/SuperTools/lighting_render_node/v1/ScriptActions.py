...
def load_asset_data(knode):
    ...
    pipe_parameter.getChild("show_path").setValue(
        inputs["show_path"], 1.0
    )
    pipe_parameter.getChild("sequence").setValue(
        inputs["sequence"], 1.0
    )
    pipe_parameter.getChild("shot").setValue(inputs["shot"], 1.0)
    pipe_parameter.getChild("global_frame_range.i0").setValue(
        inputs["frame_range"][0], 1.0
    )
    pipe_parameter.getChild("global_frame_range.i1").setValue(
        inputs["frame_range"][1], 1.0
    )
    ...
    for category in studio_nodes.CATEGORIES:
        ...
        for asset in inputs["assets"][category]:
            asset_parameter = assets_parameter.createChildNumberArray(
                asset, 2
            )
            asset_parameter.getChild("i0").setValue(
                inputs["frame_range"][0], 1.0
            )
            asset_parameter.getChild("i1").setValue(
                inputs["frame_range"][1], 1.0
            )
            # ===================================================================
    ...
    camera_producer = scenegraph.get_producer(
        kasset_node, location=camera_location
    )
    camera_producers = scenegraph.list_specific_producer(
    ...
    if not camera_producers:
        ...
        return
    pipe_parameter.getChild("render_camera").setValue(
        camera_producers[0].getFullName(), 1.0
    )
    ...
    pipe_parameter.getChild("render_location").setValue(
        render_path, 1.0
    )
...
def make_node_networks(knode, asset_inputs):
    ...
    for category in studio_nodes.CATEGORIES:
        ...
        for asset in asset_inputs[category]:
            ...
            render_node = NodegraphAPI.CreateNode(
                "Render", parent=knode
            )
            ...
            knode.getSendPort("input").connect(
                input_dot.getInputPort("input")
            )
            ...
            for driver, driven in attributes.items():
                if driver == "render_location":
                    ...
                    expression = (
                        "self.getNode().getParent().getParameter('%s.%s').getValue(1.0) + '%s' + %s"
                        % ("studio_pipe", driver, name, extension)
                    )
                else:
                    expression = (
                        "self.getNode().getParent().getParameter('%s.%s').getValue(1.0)"
                        % ("studio_pipe", driver)
                    )
...
def create_output_define(knode):
    xml_scene = os.path.join(
        os.path.dirname(__file__), "output_define.xml"
    )
...
def set_lighting_versions(knode):
    ...
    temp_version = versions.get_latest_version(
        publish_path, "lighting"
    )
    latest_version = None
    if temp_version:
    ...
    if not latest_version:
        latest_version = "None"
    pipe_parameter.getChild("latest_version").setValue(
        latest_version, 1.0
    )
    pipe_parameter.getChild("next_version").setValue(
        next_version, 1.0
    )
    render_path = os.path.join(publish_path, "render", next_version)
    pipe_parameter.getChild("render_location").setValue(
        render_path, 1.0
    )
...
def get_render_input_data(knode):
    ...
    frame_type = pipe_parameter.getChild("current_frame").getValue(
        1.0
    )
    assets_data = {}
    for assets_child in assets_parameter.getChildren():
        ...
        assets_data.setdefault(
            assets_child.getName(), [start_frame, end_frame]
        )
    show_path = pipe_parameter.getChild("show_path").getValue(1.0)
    sequence = pipe_parameter.getChild("sequence").getValue(1.0)
    shot = pipe_parameter.getChild("shot").getValue(1.0)
    global_start_frame = pipe_parameter.getChild(
        "global_frame_range.i0"
    ).getValue(1.0)
    global_end_frame = pipe_parameter.getChild(
        "global_frame_range.i1"
    ).getValue(1.0)
    next_version = pipe_parameter.getChild("next_version").getValue(
        1.0
    )
    render_input_data = {
        "assets": assets_data,

    ...
    def add_parameters(self):
        ...
        for attribute in attributes:
            if "type" not in hints[attribute]:
                child_parameter = (
                    pipe_parameter_group.createChildString(
                        attribute, ""
                    )
                )
            else:
                if hints[attribute]["type"] == "NumberArray":
                    child_parameter = (
                        pipe_parameter_group.createChildNumberArray(
                            attribute, 2
                        )
                    )
                    child_parameter.getChild("i0").setValue(
                        "1001", 1.0
                    )
                    child_parameter.getChild("i1").setValue(
                        "1025", 1.0
                    )
                if hints[attribute]["type"] == "Group":
                    child_parameter = (
                        pipe_parameter_group.createChildGroup(
                            attribute
                        )
                    )

...
def example():
    ...
    # alembic node 1
    batman_abc = NodegraphAPI.CreateNode(
        "Alembic_In", parent=root_node
    )
    ...
    abc_paramter.setValue(
        "/venture/shows/katana_tutorials/tmp/batman.abc", 1.0
    )
    abc_paramter.setUseNodeDefault(False)

    # katana primitive - plane
    primitive_plane = NodegraphAPI.CreateNode(
        "PrimitiveCreate", parent=root_node
    )
    ...
    # alembic camera
    camera_abc = NodegraphAPI.CreateNode(
        "Alembic_In", parent=root_node
    )
    ...
    abc_paramter.setValue(
        "/venture/shows/katana_tutorials/tmp/camera.abc", 1.0
    )
    abc_paramter.setUseNodeDefault(False)

    # katana primitive - camera
    katana_camera = NodegraphAPI.CreateNode(
        "CameraCreate", parent=root_node
    )
    ...
    xyz = {
        "translate": {"x": 0.80, "y": 4.61, "z": 7.45},
        "rotate": {"x": -9.74, "y": 4.58, "z": 5.98},
    }

    for k, v in xyz.items():
        for axis, value in v.items():
            axis_parameter = katana_camera.getParameter(
                "transform.%s.%s" % (k, axis)
            )

...
def add_parameters(knode):
    ...
    node_category_parameter = parameter.createChildString(
        ...
        studio_nodes.LIGHTING_ASSET_NODE_TYPE,
    )
    hint = {"readOnly": "True"}
    node_category_parameter.setHintString(str(hint))
    show_path = resources.get_show_path()
    show_path_parameter = parameter.createChildString(
        "show_path", show_path
    )
    ...
    create_button = parameter.createChildString(
...
def make_node_networks(knode, inputs):
    ...
    for puppet, contents in animation_inputs["puppets"].items():
        ...
        puppet_group = create_puppet_group(
            knode, contents, [x, y], ing
        )
        y += ing
    ...
    knode.getParameter("frame_ranage.i0").setValue(
        frame_range[0], 1.0
    )
    knode.getParameter("frame_ranage.i1").setValue(
        frame_range[1], 1.0
    )
...
def upadte_category_parameter(knode, typed_version, contents):
    ...
    category_parameter = parameter.createChildStringArray(
        contents["name"], 6
    )
...
def create_puppet_group(knode, contents, position, ing):
    ...
    merge_node = create_merge_node(
        [0, 0], puppet_group, contents["name"]
    )
    input_port = merge_node.addInputPort("input")
    look_resolve, look_assign = None, None
    if contents["category"] != "camera":
        look_resolve = NodegraphAPI.CreateNode(
            "LookFileResolve", parent=puppet_group
        )
        look_resolve.setName("%s_resolve" % contents["name"])
        look_resolve.getOutputPort("out").connect(input_port)
        look_assign = NodegraphAPI.CreateNode(
            "LookFileAssign", parent=puppet_group
        )
        ...
        klf_parameter = look_assign.getParameter(
            "args.lookfile.asset"
        )
        expression = get_expression(
            "lookdev", contents["category"], contents["name"], "i3"
        )
    ...
    alembic_node = NodegraphAPI.CreateNode(
        "Alembic_In", parent=puppet_group
    )
    ...
    expression = get_expression(
        "animation", contents["category"], contents["name"], "i5"
    )
...
def get_animation_inputs(show_path, sequence, shot, typed):
    ...
    animation_dirname = os.path.join(
        show_path, "scene", sequence, shot
    )
    if not os.path.isdir(animation_dirname):
        message = "not found any valid %s publish" % typed
        return None, message
    latest_version = versions.get_latest_version(
        animation_dirname, typed
    )
    ...
    if not os.path.isfile(manifest_path):
        message = (
            "not found any valid %s manifest (scene description)"
            % typed
        )
        return None, message
...
def update_version_action(typed, parameter, puppet, category):
    ...
    if current_version == latest_version:
        ...
        QtGui.QMessageBox.information(
            None, "information", message, QtGui.QMessageBox.Ok
        )
        return
...
def get_current_version(parameter, puppet, column):
    ...
    child_parameter = parent_parameter.getChild(
        "%s.%s" % (puppet, column)
    )

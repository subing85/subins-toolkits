...
def add_parameters(knode):
    ...
    node_category_parameter = parameter.createChildString(
        ...
        studio_nodes.GENERIC_ASSET_NODE_TYPE,
    )
    hint = {"readOnly": "True"}
    ...
    update_parameter = parameter.createChildString(
...
def add_button_action(parameter):  # not change the function name
    ...
    category_parameter = asset_parameter.createChildString(
        "category", ""
    )
    ...
    version_parameter = asset_parameter.createChildString(
        "version", "0.0.0"
    )
    remove_parameter = asset_parameter.createChildString(
        "remove", "remove"
    )
...
def get_expression(paremeter):
    ...
    expressions = [
        ...
        "'asset'",
        "self.getNode().getParent().getParameter('assets.%s.category').getValue(1.0)"
        % paremeter,
        "self.getNode().getParent().getParameter('assets.%s.name').getValue(1.0)"
        % paremeter,
        "'model'",
        "self.getNode().getParent().getParameter('assets.%s.version').getValue(1.0)"
        % paremeter,
        "self.getNode().getParent().getParameter('assets.%s.name').getValue(1.0) + '.abc'"
        % paremeter,
    ]

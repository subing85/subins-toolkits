...
def add_parameters(knode):
    ...
    node_category_parameter = parameter.createChildString(
        ...
        studio_nodes.LOOKDEV_ASSET_NODE_TYPE,
    )
    hint = {"readOnly": "True"}
    ...
    hint = {"widget": "popup", "options": studio_nodes.CATEGORIES}
    ...
    camera_category = parameter.createChildString(
...
def get_inputs(knode):
    ...
    model_category = knode.getParameter("model_category").getValue(
        1.0
    )
    model_version = knode.getParameter("model_version").getValue(1.0)
    camera_name = knode.getParameter("camera_name").getValue(1.0)
    camera_category = knode.getParameter("camera_category").getValue(
        1.0
    )
    camera_version = knode.getParameter("camera_version").getValue(
        1.0
    )
    inputs = {
        "model": {
...
def make_node_networks(knode, inputs):
    ...
    for asset in assets:
        position = [x, y]
        alembic_node = create_alembic_node(
            inputs[asset], position, asset, knode
        )
        y += ing
...
def get_expression(paremeter):
    ...
    expressions = [
        ...
        "'asset'",
        "self.getNode().getParent().getParameter('%s_category').getValue(1.0)"
        % paremeter,
        "self.getNode().getParent().getParameter('%s_name').getValue(1.0)"
        % paremeter,
        "'model'",
        "self.getNode().getParent().getParameter('%s_version').getValue(1.0)"
        % paremeter,
        "self.getNode().getParent().getParameter('%s_name').getValue(1.0) + '.abc'"
        % paremeter,
    ]

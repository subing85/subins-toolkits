...
class Asset(object):
    ...
    def create_maya_file(
        ...
        bash_file = os.path.abspath(
            os.path.join(tempfile.gettempdir(), "asset_library.py")
        ).replace("\\", "/")
        ...
        output_file = os.path.abspath(
            os.path.join(
                output_path,
                "asset_bundle_{}.{}".format(
                    current_time, self.maya_file_types[maya_type]
                ),
            )
        )
        ...
        data = [
            ...
            "\ttry:",
            "\t\tcore.saveAs('{}', typ='{}')".format(
                output_file, maya_type
            ),
        ...
        if platform.system() == "Windows":
            mayapy = os.path.abspath(
                os.path.join(maya_path, "bin/mayapy.exe")
            ).replace("\\", "/")
        ...
        for each_asset in asset_data:
            ...
            if create_type == "reference":
                core.createReference(
                    asset_path, iv=True, ns=asset_name
                )
            if create_type == "import":
                core.importFile(asset_path, iv=True, ns=asset_name)
        return True

...
from assetLibrary_maya2017.modules import studioImage
...
class Asset(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self.studio_print = studioPrint.Print(
            __name__, self.textedit_console
        )
    ...
    def setup_ui(self):
        ...
        self.verticallayout_asset = QtWidgets.QVBoxLayout(
            self.groupbox_asset
        )
        self.verticallayout_asset.setObjectName(
            "verticallayout_asset"
        )
        self.verticallayout_asset.setSpacing(10)
        self.verticallayout_asset.setContentsMargins(10, 10, 10, 10)
        self.groupbox_snapshot = QtWidgets.QGroupBox(
            self.groupbox_asset
        )
        ...
        self.horizontallayout_snapshot.setContentsMargins(
            10, 10, 10, 10
        )
        ...
        self.button_snapshot = QtWidgets.QPushButton(
            self.groupbox_snapshot
        )
        ...
        self.gridlayout_inputs.addWidget(
            self.label_filepath, 0, 0, 1, 1
        )
        ...
        self.gridlayout_inputs.addWidget(
            self.lineedit_filepath, 0, 1, 1, 1
        )
        self.pushbutton_filepath = QtWidgets.QPushButton(
            self.groupbox_asset
        )
        self.pushbutton_filepath.setObjectName("pushbutton_filepath")
        self.pushbutton_filepath.setText("...")
        self.gridlayout_inputs.addWidget(
            self.pushbutton_filepath, 0, 2, 1, 1
        )
        ...
        self.gridlayout_inputs.addWidget(
            self.label_imagepath, 1, 0, 1, 1
        )
        self.lineedit_imagepath = QtWidgets.QLineEdit(
            self.groupbox_asset
        )
        self.lineedit_imagepath.setObjectName("lineedit_imagepath")
        self.gridlayout_inputs.addWidget(
            self.lineedit_imagepath, 1, 1, 1, 1
        )
        self.pushbutton_imagepath = QtWidgets.QPushButton(
            self.groupbox_asset
        )
        self.pushbutton_imagepath.setObjectName("pushbutton_filepath")
        self.pushbutton_imagepath.setText("...")
        self.gridlayout_inputs.addWidget(
            self.pushbutton_imagepath, 1, 2, 1, 1
        )
        ...
        self.horizontallayout_label.setObjectName(
            "horizontallayout_label"
        )
        ...
        self.textedit_history = QtWidgets.QTextEdit(
            self.groupbox_asset
        )
        ...
        self.button_publish = QtWidgets.QPushButton(
            self.groupbox_asset
        )
        ...
        self.image_to_button(
            self.button_logo, log_path, self._width, self._height
        )
        ...
        self.textedit_console.setMaximumSize(
    ...
    def image_to_button(
        self, button=None, path=None, width=None, height=None
    ):
        if not button:
            button = self.button_snapshot
        if not path:
            path = os.path.join(
                resources.getIconPath(), "snapshot.png"
            )
        if not width:
            width = self._width
        if not height:
    ...
    def snapshot(self, button):
        self.studio_image.image_file = os.path.join(
        ...
        if not image_path:
            QtWidgets.QMessageBox.warning(
                self,
                "Warning",
                "Not able to process snap shot!..",
                QtWidgets.QMessageBox.Ok,
            )
            self.studio_print.display_warning("Snap shot - faild!...")
            return
        self.image_to_button(
            button, image_path, self._width, self._height
        )
